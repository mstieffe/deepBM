            ...
            for bead in bead_seq:
                ...
                if self.pad_seq:
                    for n in range(0, self.data.max['seq_len'] - len(atom_seq_dict[bead])):
                        ...
                        bonds_ndx_pa.append(np.zeros(np.array(bonds_ndx_pa[-1]).shape))
                        ...
                        dihs_ndx_pa.append(np.zeros(np.array(dihs_ndx_pa[-1]).shape))
                        ljs_ndx_pa.append(np.zeros(np.array(ljs_ndx_pa[-1]).shape))
